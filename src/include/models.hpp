        ...
        try {
            ...
            for (int i = 1; i <= N; ++i) {
                ...
                if (as_bool((primitive_value(logical_eq(get_base1(cens_x,i,"cens_x",1),0)) && primitive_value(logical_eq(get_base1(cens_y,i,"cens_y",1),0))))) {
                    ...
                    lp_accum__.add(multi_normal_log(stan::math::to_row_vector(stan::math::array_builder<double >().add(get_base1(x,i,"x",1)).add(get_base1(y,i,"y",1)).array()),mu_xy,T_xy));
                } else if (as_bool((primitive_value(logical_eq(get_base1(cens_x,i,"cens_x",1),0)) && primitive_value(logical_eq(get_base1(cens_y,i,"cens_y",1),1))))) {

                    lp_accum__.add((normal_log(get_base1(x,i,"x",1),mu_x,sigma_x) + normal_cdf_log(get_base1(y,i,"y",1),(mu_y + ((rho_xy * (sigma_y / sigma_x)) * (get_base1(x,i,"x",1) - mu_x))),(sigma_y * sqrt((1 - square(rho_xy)))))));
                } else if (as_bool((primitive_value(logical_eq(get_base1(cens_x,i,"cens_x",1),1)) && primitive_value(logical_eq(get_base1(cens_y,i,"cens_y",1),0))))) {

                    lp_accum__.add((normal_log(get_base1(y,i,"y",1),mu_y,sigma_y) + normal_cdf_log(get_base1(x,i,"x",1),(mu_x + ((rho_xy * (sigma_x / sigma_y)) * (get_base1(y,i,"y",1) - mu_y))),(sigma_x * sqrt((1 - square(rho_xy)))))));
                } else if (as_bool((primitive_value(logical_eq(get_base1(cens_x,i,"cens_x",1),1)) && primitive_value(logical_eq(get_base1(cens_y,i,"cens_y",1),1))))) {

                    lp_accum__.add(log(binormal_cdf(((get_base1(x,i,"x",1) - mu_x) / sigma_x),((get_base1(y,i,"y",1) - mu_y) / sigma_y),rho_xy, pstream__)));
                }
                if (as_bool(logical_eq(get_base1(cens_x,i,"cens_x",1),0))) {

                    lp_accum__.add(multi_normal_log(stan::math::to_row_vector(stan::math::array_builder<double >().add(get_base1(x,i,"x",1)).add(get_base1(z,i,"z",1)).array()),mu_xz,T_xz));
                } else if (as_bool(logical_eq(get_base1(cens_x,i,"cens_x",1),1))) {

                    lp_accum__.add((normal_log(get_base1(z,i,"z",1),mu_z,sigma_z) + normal_cdf_log(get_base1(x,i,"x",1),(mu_x + ((rho_xz * (sigma_x / sigma_z)) * (get_base1(z,i,"z",1) - mu_z))),(sigma_x * sqrt((1 - square(rho_xz)))))));
                }
                if (as_bool(logical_eq(get_base1(cens_y,i,"cens_y",1),0))) {

                    lp_accum__.add(multi_normal_log(stan::math::to_row_vector(stan::math::array_builder<double >().add(get_base1(y,i,"y",1)).add(get_base1(z,i,"z",1)).array()),mu_yz,T_yz));
                } else if (as_bool(logical_eq(get_base1(cens_y,i,"cens_y",1),1))) {

                    lp_accum__.add((normal_log(get_base1(z,i,"z",1),mu_z,sigma_z) + normal_cdf_log(get_base1(y,i,"y",1),(mu_y + ((rho_yz * (sigma_y / sigma_z)) * (get_base1(z,i,"z",1) - mu_z))),(sigma_y * sqrt((1 - square(rho_yz)))))));
                }
            }
        } catch (const std::exception& e) {
            stan::lang::rethrow_located(e, current_statement_begin__, prog_reader__());
