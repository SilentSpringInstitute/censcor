...
The `censR` package provides routines for performing Bayesian estimation of the correlation between two censored variables. The inference is conducted with `Stan`, which uses the `No-U-Turn Hamiltonian Monte-Carlo` algorithm for drawing from the posterior distribution.

## Usage

The main function in `censR` is `censcor`, which computes correlations between two variables. `censcor` takes as arguments a formula describing the correlation to estimate and a data frame. The formula syntax is modeled after the censoring syntax in `brms`.
